...
function testPage(pageNum){
    $.ajax({
        type: "POST",
        url: "/academicreport/selectByUserId.do",
...
function display(data) {
    ...
    for (i in data) {
        ...
        str += "<tr class='text-c'>" +
            ...
            "<td name='reportTitle'width='150'>" + data[i].reportTitle + "</td>" +
            "<td name='beginTime'>" + beginTime + "</td>" +
            "<td name='overTime' width='130'>" + overTime + "</td>" +
            "<td name='hostUnit' width='100'>" + data[i].hostUnit + "</td>" +
            "<td name='hostPlace' width='100'>" + data[i].hostPlace + "</td>" +
            "<td name='attendNum' width='100'>" + data[i].attendNum + "</td>" +
            "<td ><button  class='editAcad btn btn-primary radius ' value='"+data[i].repId+"' onclick="+"jumpEdit(this.value)"+">编辑</button>" +
            "<button class='deleteAcad btn btn-danger radius ' value='"+data[i].repId+"' onclick="+"jumpDelete(this.value)"+">删除</button></td>"+
            "</tr>";
    }
